...
*/
contract RewardsRecoveryStrategy_governance is BaseStrategy, CurveSwapper, UniswapSwapper, TokenSwapPathRegistry {
    ...
    function harvest() external whenNotPaused returns (HarvestData memory) {
        ...
        harvestData.cvxCrvHarvested = crvToken.balanceOf(address(this)); // crv returned instead of cvxCrv
        harvestData.cvxHarvsted = cvxToken.balanceOf(address(this));

        // 2. Transfer obtaned tokens to governance
        crvToken.transfer(governance, harvestData.cvxCrvHarvested);
        cvxToken.transfer(governance, harvestData.cvxHarvsted);
    }
}